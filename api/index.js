...
app.get('/download/:what',(req,res) => {
	...
	emitter.on("get_json_file", function getJsonFile(programId, outputFile, numAttempts) {
		//parse file type
		var fileType = programId.substring(0, 2);
		programId = programId.substring(3, programId.length);
		...
		try{
			...
			https.get(jsonUrl, function(res) {
				...
				file.on("finish", function() {
					...
					//switch on index header
					switch(fileType){
						case GRN_VIDEO:
							console.log("\nFiles of type 'GRN Video' not supported yet\n");
							downloadNext(outputFile);
							break;
						case JESUS_FILM:
							console.log("\nFiles of type 'Jesus Film' not supported yet\n");
							downloadNext(outputFile);
							break;
						case IMAGE:
							console.log("\nFiles of type 'Image' not supported yet\n");
							downloadNext(outputFile);
							break;
						case LANG_SAMPLE:
							console.log("\nFiles of type 'Language Sample' not supported yet\n");
							downloadNext(outputFile);
							break;
						case GRN_AUDIO:
							//two cases for audio files
						...
						default:
							wasError = true;
							downloadNext(outputFile); //enable the download to continue
							console.log("unknown file type " + fileType + " from " + programId);
					}
				} catch(error){
					//alert of error and carry on
					console.log("\njson file corrisponding to " + jsonFilename + " is invalid. Resulted in error ");
					console.log(error + "\n"); 
					wasError = true;
					downloadNext(outputFile);
				}
	...
	emitter.on("get_mp3_file", function getMp3File(zipFilename, programId, jsonFilename, jsonData, audioDirName, outputFile, numAttempts) {
		const download = require('download');
	...
	//get the language. This can vary if there is puntuation. Remove any
		...
		.catch(error => {
			...
			if(numAttempts < MAX_ATTEMPTS){
				...
				wasError = true;
				downloadNext(outputFile); //let the program carry on
			}
		});
	});
	...
	function downloadNext(outputFile){
		//keep track of how many programs have been processed and only finish after last
		numProgsDone++;
